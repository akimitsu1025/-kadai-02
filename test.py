import os
from selenium.webdriver import Chrome, ChromeOptions # 「selenium」の「webdriver」の「Chrome」と「ChromeOptions」をインポートするってこと？
import time
import pandas as pd
import datetime
from selenium.webdriver.chrome.webdriver import WebDriver # 「selenium」の「webdriver」の「Chrome」の「webdriver」の「Webdriber」をインポートするってこと？「webdriver」が2回？「webdriver」と「WebDriver」の違いは？
from selenium.webdriver.remote.webelement import WebElement # 「selenium」の「webdriver」の「remote」の「webelement」の「WebElement」をインポートするってこと？
from webdriver_manager.chrome import ChromeDriverManager #「webdriver_manager」の「chrome」の「ChromeDriverManager」をインポートするってこと？
LOG_FILE_PATH = "./log/log_{datetime}.log" #どういうこと？
EXP_CSV_PATH="./exp_list_{search_keyword}_{datetime}.csv" #どういうこと？
...
def set_driver(driver_path, headless_flg):
    ...
    return Chrome(ChromeDriverManager().install(), options=options) #　解説いただきたい
### ログファイルおよびコンソール出力
def log(txt):
    ...
    with open(log_file_path, 'a', encoding='utf-8_sig') as f:
        f.write(logStr + '\n')
    print(logStr)
def find_table_target_word(th_elms, td_elms, target:str): #この関数の意義と引数の意味とは？
    # tableのthからtargetの文字列を探し一致する行のtdを返す
    for th_elm,td_elm in zip(th_elms,td_elms): # 解説いただきたい
        if th_elm.text == target: # 解説いただきたい
            return td_elm.text # 解説いただきたい
### main処理
def main():
    log("処理開始")
